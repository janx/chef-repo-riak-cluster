...
Vagrant.configure("2") do |config|
  ...
  config.vm.box = "opscode-ubuntu-12.10-i386"

  # The url from where the 'config.vm.box' box will be fetched if it
  # doesn't already exist on the user's system.
  config.vm.box_url = "https://opscode-vm.s3.amazonaws.com/vagrant/opscode_ubuntu-12.10-i386_provisionerless.box"
  ...
  config.vm.hostname = "vagrant-riak1"
  ...
  config.vm.network :forwarded_port, guest: 8098, host: 8091
  config.vm.network :forwarded_port, guest: 8087, host: 8081
  ...
  config.vm.provision :chef_client do |chef|
    chef.chef_server_url = "https://api.opscode.com/organizations/seofusion"
    chef.validation_client_name = "seofusion-validator"
    chef.validation_key_path = ".chef/seofusion-validator.pem"
